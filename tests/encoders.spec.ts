...
describe('encoders', () => {
  ...
  describe('assertAccuracy', () => {
    ...
    function assertAccuracy(
      ...
      it(`${paramName}: ${expectedValue}`, async () => {
        ...
        if (!actualValue.eq(expectedValue)) {
          ...
          expect(
            absoluteError.lte(maxAbsoluteError) ||
    ...
    const calcInitialRate = (x: Decimal) =>
      decodeScaleInitialRate(
        BnToDec(
          decodeFloatInitialRate(
            encodeFloatInitialRate(encodeScaleInitialRate(x))
          )
        )
      );
    const calcMultiFactor = (x: Decimal) =>
      decodeScaleMultiFactor(
        BnToDec(
          decodeFloatMultiFactor(
            encodeFloatMultiFactor(encodeScaleMultiFactor(x))
          )
        )
      );
